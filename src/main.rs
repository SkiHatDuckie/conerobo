...
fn setup(mut commands: Commands, asset_server: Res<AssetServer>) {
    ...
    let file_button_dropdown_node = DropdownNodeBundle {
        style: Style {
            justify_content: JustifyContent::FlexStart,
            position_type: PositionType::Absolute,
            position: UiRect {
    ...
    commands
        ...
        .with_children(|parent| {
            parent
                .spawn_bundle(file_menu_node)
                .with_children(|parent| {
                    parent
                        .spawn_bundle(file_button)
                        .with_children(|parent| {
                            parent.spawn_bundle(file_button_dropdown_node);
...
fn button_system(
    ...
    for (interaction, mut _color, children) in &mut interaction_query {
        match *interaction {
            Interaction::Clicked => {
                if let Ok(mut vis_dropdown) = dropdown_query.get_mut(children[0]) {
                    vis_dropdown.is_visible = true
                }
            }
            Interaction::Hovered => {}
            Interaction::None => {}
